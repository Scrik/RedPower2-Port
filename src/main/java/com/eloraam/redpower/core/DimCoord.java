...
public class DimCoord
{
	 ...
	 */
	public DimCoord(int x, int y, int z, int dimension)
	{
		xCoord = x;
		yCoord = y;
		zCoord = z;
	 ...
	 */
	public DimCoord translate(int x, int y, int z)
	{
		xCoord += x;
		yCoord += y;
		zCoord += z;
	 ...
	 * @return translated Coord4D
	 */
	public DimCoord getFromSide(ForgeDirection side)
	{
	 ...
	 * @return translated Coord4D
	 */
	public DimCoord getFromSide(ForgeDirection side, int amount)
	{
		return new DimCoord(xCoord+(side.offsetX*amount), yCoord+(side.offsetY*amount), zCoord+(side.offsetZ*amount), dimensionId);
	}
	 ...
	 */
	public static DimCoord get(TileEntity tileEntity)
	{
		return new DimCoord(tileEntity.xCoord, tileEntity.yCoord, tileEntity.zCoord, tileEntity.getWorldObj().provider.dimensionId);
	}
	 ...
	 */
    public static DimCoord read(NBTTagCompound tag)
    {
        return new DimCoord(tag.getInteger("x"), tag.getInteger("y"), tag.getInteger("z"), tag.getInteger("id"));
    }
	 ...
	 */
	public static DimCoord read(ByteBuf dataStream)
	{
		return new DimCoord(dataStream.readInt(), dataStream.readInt(), dataStream.readInt(), dataStream.readInt());
	}
	 ...
	 */
	public DimCoord difference(DimCoord other)
	{
		return new DimCoord(xCoord-other.xCoord, yCoord-other.yCoord, zCoord-other.zCoord, dimensionId);
	}
	 ...
	 */
	public ForgeDirection sideDifference(DimCoord other)
	{
		DimCoord diff = difference(other);
	 ...
	 */
	public int distanceTo(DimCoord obj)
	{
	 ...
	 * @return this Coord4D
	 */
	public DimCoord step(ForgeDirection side)
	{
	...
	{
		return new DimCoord(xCoord, yCoord, zCoord, dimensionId);
	}
	...
	public boolean equals(Object obj)
	{
		return obj instanceof DimCoord &&
				((DimCoord)obj).xCoord == xCoord &&
				((DimCoord)obj).yCoord == yCoord &&
				((DimCoord)obj).zCoord == zCoord &&
				((DimCoord)obj).dimensionId == dimensionId;
	}
