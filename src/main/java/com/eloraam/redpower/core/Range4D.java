...
public class Range4D {
	...
	public static Range4D getChunkRange(TileEntity entity) {
		int radius = FMLCommonHandler.instance().getMinecraftServerInstance().getConfigurationManager().getViewDistance();
		return new Range4D(new Chunk3D(DimCoord.get(entity))).expandChunks(radius);
	}
	...
	public boolean intersects(Range4D range) {
		return (xMax + 1 - 1.E-05D > range.xMin)
				&& (range.xMax + 1 - 1.E-05D > xMin)
				&& (yMax + 1 - 1.E-05D > range.yMin)
				&& (range.yMax + 1 - 1.E-05D > yMin)
				&& (zMax + 1 - 1.E-05D > range.zMin)
	...
	public String toString() {
		return "[Range4D: " + xMin + ", " + yMin + ", " + zMin + ", " + xMax
				+ ", " + yMax + ", " + zMax + ", dim=" + dimensionId + "]";
	}
