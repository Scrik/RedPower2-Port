...
public class RenderCPU extends RenderCustomBlock {
	...
	public void renderWorldBlock(RenderBlocks renderblocks, IBlockAccess iba, int i, int j, int k, int md) {
		TileCPU cpu = (TileCPU) CoreLib.getTileEntity(iba, i, j, k, TileCPU.class);
		if (cpu != null) {
			this.context.setDefaults();
			this.context.readGlobalLights(iba, i, j, k);
			//this.context.bindTexture("/eloraam/control/control1.png");
			IIcon sideIcon = RedPowerControl.blockPeripheral.getIcon(0, md);
			IIcon backIcon = RedPowerControl.blockPeripheral.getIcon(1, md);
			IIcon topIcon = RedPowerControl.blockPeripheral.getIcon(2, md);
			IIcon bottomIcon = RedPowerControl.blockPeripheral.getIcon(3, md);
